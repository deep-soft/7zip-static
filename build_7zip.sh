...
git ls-files -z | xargs -0 unix2dos -q --allow-chown && QUILT_PATCHES=../patches quilt push -a && \
...
make -j$(nproc) \
  ...
  CFLAGS_WARN_WALL="-Wall -Wextra" COMPL_STATIC=1 $MAKE_OPTS && \

find . -type f -name '7zzs' -exec cp -va {} 7zz \; && tar -cJvf ../7zz-linux-$PLATFORM.tar.xz 7zz
